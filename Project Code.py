...
class OrderCostCalculator:
    def __init__(self):
        ...
        Label(window, text = "$5.00 + 8.25% Sales Tax in Texas").grid(row = 7, column = 7, sticky = W)
        Label(window, text = "Compute Total Cost").grid(row = 8, column = 7, sticky = W)
        ...
        Button(window, text = "Compute Total Cost", command = self.computeTotalCost).grid(row = 8, column = 7, sticky = E)
        
        window.mainloop() # Create an event loop
        
    def getMonthlyPayment(self, Subtotal, SalesTax, numberOfYears):
