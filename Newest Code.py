...
def DelivWindow():
    ...
    DelivMethod1 = StringVar()
    DelivOption = [
        '',
        'Ship', 
        'Pickup'
        ]
    OptionMenu(Deliv, DelivMethod1, *DelivOption).grid(row = 2, column = 2)
    
    def DelivChoice():
        Deliv1 = DelivMethod1.get()
        ShipAdd = str('Ship')
        Pickup = str('Pickup')
        if Deliv1 == ShipAdd:
            ShipAddress()
            ...
            Label(Deliv, text = 'Please select a delivery option').grid(row = 3, column = 1, sticky = E)
    ...
    def ShipAddress():
        ...
        OptionMenu(Deliv, DelivMethod4, *stateSelect).grid(row = 4, column = 2, sticky = W)
        zipCode = IntVar()
        Entry(Deliv, textvariable = zipCode).grid(row = 5, column = 2, sticky = W)
...
def confirmationWindow(): #create window to confirm order being placed by the user
    ...
    Label(conf, text = "Your order has been received!", foreground='White', background='Purple', font = ('Times New Roman', 30)).grid(row = 3, column = 1, sticky = N)
